...
seu servidor de arquivos. Para tentar resolver este problema, o Administrador de Rede 
...
separada, que será chamada pelo programa principal. O cálculo do percentual de uso também 
...
with open ("relatório.txt", "w") as arquivo:
	...
	for cadaUsuario in usuarios:
		percentagem = "{:.2f}".format(round(cadaUsuario[3], 2))
		arquivo.write(str(cadaUsuario[0])+'\t'+"{:<15}"
			.format(cadaUsuario[1])+'\t'+"{:<16}".format(percentagem)+'MB'+'\t'+"{:.2f}"
			.format(cadaUsuario[4])+'%'+'\n')
	clean()
	arquivo.write("\n\nEspaço Total Ocupado: {:.2f} MB".format(total) )
   	arquivo.write("\n\nEspaço médio Ocupado: {:.2f} MB".format(media))
   	arquivo.close()

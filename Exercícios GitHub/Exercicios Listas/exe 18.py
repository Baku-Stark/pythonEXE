...
mostrando uma breve mensagem de aviso, e voltando a pedir outro número. Após o final da 
votação, o programa deverá exibir: 

	O total de votos computados;
	Os númeos e respectivos votos de todos os jogadores que receberam votos;
	O percentual de votos de cada um destes jogadores;
	O número do jogador escolhido como o melhor jogador da partida, juntamente com o 
...
while True:
	while True:
		opcao = int(input('Escolha a sua opção de voto \nr: '))
		print('')
...
for s in votos:
	print('{:>3}{:>12}   {:.2f}%'.ljust(35).format(votos[cont], s, (s * 100) / sum(votos)))
	if s > melhor:
		melhor = s
		melhorPly = votos[cont]
		perc = (s * 100) / sum(votos)
	cont += 1
print('-' *45)
clear()
print('Total: {}'.format(sum(votos)))
print('\a O Sistema Operacional mais votador: {}'.format(melhorPly))
